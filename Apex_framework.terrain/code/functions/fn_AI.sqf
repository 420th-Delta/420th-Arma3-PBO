...
if (_mode isEqualTo 'GROUP') exitWith {
	...
	if (_role in ['FINAL','FINAL_ARMOR','FINAL_AIR']) then {
		...
		if (_reports isNotEqualTo []) then {
			...
			private _candidate = if (_role isEqualTo 'FINAL_AIR') then {_focus} else {_focus getPos [_distance,_focus getDir _leader]};
	...
	if (!isNull _parent && {_parent isKindOf 'Air'}) exitWith {
		if (_role isEqualTo 'FINAL_AIR' && {local _parent}) then {
			private _fire = _group getVariable ['QS_AI_GRP_fireMission',[]];
			if (_fire isNotEqualTo [] && {_now > (_fire param [1,1e10])}) then {
				_group setVariable ['QS_AI_GRP_fireMission',nil,QS_system_AI_owners]; _fire = [];
			};
			if (_fire isEqualTo [] && {!(_parent getVariable ['QS_AI_PLANE_fireMission',FALSE])}) then {
				private _orbit = _goal getPos [600 + random 400,random 360];
				_orbit set [2,[100,500] select (_parent isKindOf 'Plane')];
				_group move _orbit;
			};
		};
	};
...
// Count personnel once: infantry, armor/static crews, UAV crew, aircraft pilots
...
private _fn_force = {
	...
	{_assigned append (crew _x);} forEach (missionNamespace getVariable ['QS_enemyCasArray2',[]]);
...
if (_mode isEqualTo 'INIT') exitWith {
	...
	_state = createHashMapFromArray [
		...
		['finalNext',1e10],['finalEvents',0],['finalArmor',0],['finalRotary',0],['finalClosed',FALSE],['finalReports',[]],
...
if (_mode isEqualTo 'TICK') exitWith {
	...
	if (_now >= (_state get 'nextMobilize') && {diag_fps >= 12} && {_factor <= 0 || {_humans isNotEqualTo [] && {_state get 'engaged'}}}) then {
		...
		private _cas = [];
		if (_final) then {
			{_cas append (crew _x);} forEach (missionNamespace getVariable ['QS_enemyCasArray2',[]]);
			{_groups pushBackUnique (group _x);} forEach _cas;
		};
		private _assigned = 0;
		{
			private _group = _x;
			...
			if (_assigned < ([2,4] select _final) && {_needs} &&
				...
				{!(_group getVariable ['QS_primaryPressure_building',FALSE])} &&
				{(_members findIf {_x call _fn_exempt || {!(_x in _roster) && {!(_x in _cas)}} || {_x isEqualTo (_state get 'commander')}}) < 0} &&
	...
	if (_final) then {
		...
		if (_now >= (_state get 'finalNext')) then {
			...
			if (_ready && {_reports isNotEqualTo []}) then {
				...
				// Regional reserves: at most one armor and one rotary admission.
				...
				if (_roll >= 0.9 && {count _humans >= 16} && {(_state get 'finalRotary') < 1} &&
					{(_area findIf {(objectParent _x) isKindOf 'Helicopter'}) < 0}) then {
					_delivery = 'ROTARY'; _quantity = 4;
				};
			};
		};
	} else {
	...
	if (_final) then {
		...
		if (_delivery isEqualTo 'ROTARY') then {_state set ['finalRotary',1];};
	};
...
if (_mode isEqualTo 'WORK') exitWith {
	...
	if (_delivery isEqualTo 'ROTARY') exitWith {
		...
		if (call _fn_admit) then {
			...
			if (_objects isNotEqualTo []) then {
				...
				_state set ['used',(_state get 'used') + count (crew _aircraft)];
				['REGISTER',_flight,'AO'] call QS_fnc_combatAir;
				[_flight,_goal,'FINAL_AIR',_key] call _fn_activate;
			};
		};
		_state set ['reserved',0];
	};
...
// COMBAT_AIR_CONTROLLER_BEGIN
// Native combat flights use their existing owner-local group pass. No aircraft,
// projectiles, sensors or permanent scheduled loops are created here.
missionNamespace setVariable ['QS_fnc_combatAir',{
	params [['_mode','GROUP']];
	// COMBAT_AIR_POLICIES_BEGIN
	private _fn_pickClass = {
		params ['_combat','_transport','_roll'];
		if (_combat <= 0 && {_transport <= 0}) exitWith {''};
		if (_transport <= 0) exitWith {'COMBAT'};
		if (_combat <= 0) exitWith {'TRANSPORT'};
		['TRANSPORT','COMBAT'] select (_roll < 0.8)
	};
	private _fn_role = {
		params ['_plane','_airWeapons','_groundWeapons'];
		['CAS','CAP'] select (_plane && {_airWeapons > 0} && {_airWeapons >= _groundWeapons})
	};
	private _fn_classify = {
		params ['_combatClass','_pylonWeapon','_pilotWeapon'];
		['TRANSPORT','COMBAT'] select (_combatClass || {_pylonWeapon} || {_pilotWeapon})
	};
	private _fn_lease = {
		params ['_now','_until','_valid','_sameActivity'];
		_valid && {_sameActivity} && {_now < _until}
	};
	private _fn_groundAvailable = {
		params ['_now','_role','_busyUntil','_groundUntil','_scopeValid'];
		_role isEqualTo 'CAS' && {_scopeValid} && {_now >= _busyUntil} && {_now >= _groundUntil}
	};
	private _fn_airDeadline = {
		params ['_now','_role','_previous'];
		if (_role isEqualTo 'CAS' && {_previous > _now}) exitWith {_previous};
		_now + 45
	};
	private _fn_airAvailable = {
		params ['_role','_now','_groundPriority','_combat','_threat'];
		_role isEqualTo 'CAP' || {_now >= _groundPriority} || {_combat > 0} || {_threat}
	};
	// COMBAT_AIR_POLICIES_END
	if (_mode isEqualTo 'PICK_CLASS') exitWith {(_this select [1]) call _fn_pickClass};
	if (_mode isEqualTo 'ROLE') exitWith {(_this select [1]) call _fn_role};
	if (_mode isEqualTo 'CLASSIFY') exitWith {(_this select [1]) call _fn_classify};
	if (_mode isEqualTo 'LEASE') exitWith {(_this select [1]) call _fn_lease};
	if (_mode isEqualTo 'AVAILABLE') exitWith {
		params ['','_provider'];
		private _group = group _provider;
		if (!(_group getVariable ['QS_combatAir_managed',FALSE])) exitWith {TRUE};
		private _scopeValid = (_group getVariable ['QS_combatAir_scope','']) isNotEqualTo 'DEFENSE' ||
			{missionNamespace getVariable ['QS_defendControl_active',FALSE] &&
			{(_group getVariable ['QS_combatAir_defenseEpoch',-1]) isEqualTo (missionNamespace getVariable ['QS_defendControl_epoch',0])}};
		[serverTime,_group getVariable ['QS_combatAir_role','CAS'],(_group getVariable ['QS_combatAir_busyUntil',0]) max (_group getVariable ['QS_combatAir_groundCooldownUntil',0]),
			_group getVariable ['QS_combatAir_groundUntil',0],_scopeValid] call _fn_groundAvailable
	};
	params ['','_group'];
	if (_mode isEqualTo 'START') exitWith {
		params ['','','_target','_ends'];
		if (!(_group getVariable ['QS_combatAir_managed',FALSE])) exitWith {TRUE};
		private _accepted = FALSE;
		// Admission runs atomically on the flight owner. Recheck a delayed HC
		// request before creating a laser or taking over an interception/egress.
		isNil {
			private _request = _group getVariable ['QS_AI_GRP_fireMission',[]];
			private _occupied = serverTime < (_group getVariable ['QS_combatAir_groundUntil',0]);
			_accepted = local _group && {local (vehicle (leader _group))} &&
				{alive (leader _group)} && {alive _target} && {!captive _target} &&
				{serverTime < _ends} && {(_request param [0,objNull]) isEqualTo _target} &&
				{['AVAILABLE',leader _group] call QS_fnc_combatAir};
			if (_accepted) then {
				{_x doTarget objNull;} forEach (units _group);
				_group setVariable ['QS_combatAir_target',objNull,FALSE];
				_group setVariable ['QS_combatAir_busyUntil',0,TRUE];
				_group setVariable ['QS_combatAir_airUntil',0,FALSE];
				_group setVariable ['QS_combatAir_groundPriorityUntil',serverTime + 90,FALSE];
				_group setVariable ['QS_combatAir_groundUntil',_ends + 30,TRUE];
			} else {
				if (!_occupied && {(_request param [0,objNull]) isEqualTo _target}) then {
	...
	if (_mode isEqualTo 'EGRESS') then {_group setVariable ['QS_combatAir_groundUntil',0,TRUE];};
	if (isNull _group || {!local _group}) exitWith {FALSE};
	private _pilot = leader _group;
	private _aircraft = vehicle _pilot;
	if (!alive _pilot || {!alive _aircraft} || {!(_aircraft isKindOf 'Air')} ||
		{unitIsUav _aircraft} || {!local _aircraft} || {(WEST getFriend (side _group)) >= 0.6} ||
		{((units _group) findIf {isPlayer _x || {captive _x} || {!isNull (_x getVariable ['bis_fnc_moduleRemoteControl_owner',objNull])}}) >= 0}) exitWith {FALSE};
	private _now = serverTime;
	// Classify installed ordnance, excluding countermeasures and smoke. Count
	// missile/bomb mounts for the native jet's role; do not change its loadout.
	private _fn_weaponInfo = {
		params ['_vehicle'];
		private _air = 0; private _ground = 0; private _pylon = FALSE; private _pilotGun = FALSE;
		private _pylons = getPylonMagazines _vehicle;
		{
			if (_x isNotEqualTo '') then {
				private _ammo = configFile >> 'CfgAmmo' >> getText (configFile >> 'CfgMagazines' >> _x >> 'ammo');
				private _simulation = toLowerANSI (getText (_ammo >> 'simulation'));
				if (_simulation in ['shotmissile','shotrocket','shotshell','shotbullet','shotbomb','shotsubmunitions']) then {
					_pylon = TRUE;
					if ((getNumber (_ammo >> 'airLock')) >= 2) then {_air = _air + 1;} else {_ground = _ground + 1;};
				};
			};
		} forEach _pylons;
		{
			...
			if (_simulation in ['shotmissile','shotrocket','shotshell','shotbullet','shotbomb','shotsubmunitions']) then {
				_pilotGun = TRUE;
				// Fixed-loadout aircraft may have no pylons. Only missile/bomb
				// magazines influence CAP/CAS; the cannon is useful in either role.
				if (_pylons isEqualTo [] && {_simulation in ['shotmissile','shotrocket','shotbomb']}) then {
					if ((getNumber (_ammo >> 'airLock')) >= 2) then {_air = _air + 1;} else {_ground = _ground + 1;};
				};
			};
		} forEach (_vehicle magazinesTurret [-1]);
		[_air,_ground,_pylon,_pilotGun]
	};
	if (_mode isEqualTo 'REGISTER') exitWith {
		params ['','',['_scope','AO']];
		private _info = [_aircraft] call _fn_weaponInfo;
		private _role = [_aircraft isKindOf 'Plane',_info # 0,_info # 1] call _fn_role;
		_group setVariable ['QS_combatAir_managed',TRUE,TRUE];
		private _flights = missionNamespace getVariable ['QS_combatAir_groups',[]];
		_flights pushBackUnique _group;
		missionNamespace setVariable ['QS_combatAir_groups',_flights,FALSE];
		_group enableAttack TRUE; _group setSpeedMode 'FULL';
		_group setCombatMode 'RED';
		{_x enableAIFeature ['AUTOCOMBAT',TRUE]; _x enableAIFeature ['TARGET',TRUE]; _x enableAIFeature ['AUTOTARGET',TRUE];} forEach (units _group);
		// Native jet/rotary spawners retain their existing handler ID. Defense
		// aircraft without that hook receive it once, including armed VTOLs.
		if (isNil {_aircraft getVariable 'QS_combatAir_missileEH'}) then {
			private _eh = _aircraft addEventHandler ['IncomingMissile',QS_fnc_AIXMissileCountermeasure];
			_aircraft setVariable ['QS_combatAir_missileEH',_eh,FALSE];
		};
		_group setVariable ['QS_combatAir_role',_role,TRUE];
		_group setVariable ['QS_combatAir_scope',_scope,TRUE];
		_group setVariable ['QS_combatAir_defenseEpoch',missionNamespace getVariable ['QS_defendControl_epoch',0],TRUE];
		_group setVariable ['QS_combatAir_busyUntil',0,TRUE];
		_group setVariable ['QS_AI_GRP_HC_EXCLUDED',TRUE,TRUE];
		_group setVariable ['QS_AI_GRP',TRUE,QS_system_AI_owners];
		if (_aircraft isKindOf 'Plane') then {
			_group setVariable ['QS_AI_GRP_CONFIG',[['AIR_PATROL_CAS','AIR_PATROL_FIGHTER'] select (_role isEqualTo 'CAP'),'',_aircraft],QS_system_AI_owners];
			_group setVariable ['QS_AI_GRP_DATA',[TRUE,_now],QS_system_AI_owners];
			_group setVariable ['QS_AI_GRP_TASK',[_role,missionNamespace getVariable ['QS_AOpos',[0,0,0]],_now,-1],QS_system_AI_owners];
		};
		// CAP is not advertised as a ground-fire provider. Existing CAS provider
		// registration remains authoritative, including the rotary patrol path.
		if (_role isEqualTo 'CAP' && {isServer}) then {
			missionNamespace setVariable ['QS_AI_supportProviders_CASPLANE',(missionNamespace getVariable ['QS_AI_supportProviders_CASPLANE',[]]) - (units _group),QS_system_AI_owners];
		};
		if (_scope isEqualTo 'DEFENSE' && {_role isEqualTo 'CAP'}) then {
			// Only this flight's old HQ attack waypoint is replaced by its orbit.
			_group lockWP FALSE;
			for '_i' from ((count (waypoints _group)) - 1) to 0 step -1 do {deleteWaypoint [_group,_i];};
		};
		if (isNil {_aircraft getVariable 'QS_combatAir_hitEH'}) then {
			private _eh = _aircraft addEventHandler ['Hit',{
				params ['_vehicle','_causedBy','_damage',['_instigator',objNull]];
				private _attacker = vehicle ([_causedBy,_instigator] select (!isNull _instigator));
				if (!isNull _attacker && {_attacker isKindOf 'Air'}) then {
					_vehicle setVariable ['QS_combatAir_threat',[_attacker,serverTime + 30],FALSE];
				};
			}];
			_aircraft setVariable ['QS_combatAir_hitEH',_eh,FALSE];
		};
		diag_log format ['[CombatAir] REGISTER role=%1 scope=%2 type=%3',_role,_scope,typeOf _aircraft];
		TRUE
	};
	private _managed = _group getVariable ['QS_combatAir_managed',FALSE];
	if (!_managed) exitWith {FALSE};
	private _scopeValid = !((_group getVariable ['QS_combatAir_scope','']) isEqualTo 'DEFENSE' &&
		{!(missionNamespace getVariable ['QS_defendControl_active',FALSE]) ||
		{(_group getVariable ['QS_combatAir_defenseEpoch',-1]) isNotEqualTo (missionNamespace getVariable ['QS_defendControl_epoch',0])}});
	if (!_scopeValid) exitWith {
		{_x doTarget objNull;} forEach (units _group);
		_group setVariable ['QS_combatAir_target',objNull,FALSE];
		_group setVariable ['QS_combatAir_busyUntil',0,TRUE];
		FALSE
	};
	private _role = _group getVariable ['QS_combatAir_role','CAS'];
	private _center = missionNamespace getVariable ['QS_AOpos',[0,0,0]];
	if (missionNamespace getVariable ['QS_defendActive',FALSE]) then {_center = missionNamespace getVariable ['QS_HQpos',_center];};
	private _activity = [missionNamespace getVariable ['QS_defendActive',FALSE],+_center];
	private _sameActivity = (_group getVariable ['QS_combatAir_activity',[]]) isEqualTo _activity;
	// An accepted ground pass owns movement until its native cleanup runs.
	// Withdrawing a pass on an activity change lets that cleanup delete its laser.
	private _fire = _group getVariable ['QS_AI_GRP_fireMission',[]];
	if (_mode isEqualTo 'GROUP' && {_fire isNotEqualTo []}) exitWith {
		if ((_group getVariable ['QS_combatAir_activity',[]]) isNotEqualTo [] && {!_sameActivity} ||
			{_now > (_fire param [1,1e10])}) then {
			_group setVariable ['QS_AI_GRP_fireMission',nil,QS_system_AI_owners];
		};
		TRUE
	};
	if (!_sameActivity) then {
		{_x doTarget objNull;} forEach (units _group);
		_group setVariable ['QS_combatAir_activity',_activity,FALSE];
		_group setVariable ['QS_combatAir_target',objNull,FALSE];
		_group setVariable ['QS_combatAir_commitUntil',0,FALSE];
		_group setVariable ['QS_combatAir_busyUntil',0,TRUE];
		_group setVariable ['QS_combatAir_egressUntil',0,FALSE];
		_group setVariable ['QS_combatAir_groundPriorityUntil',0,FALSE];
		_group setVariable ['QS_combatAir_groundCooldownUntil',0,TRUE];
		_group setVariable ['QS_combatAir_airUntil',0,FALSE];
	};
	if (_mode isEqualTo 'EGRESS') exitWith {
		params ['','','_point'];
		private _primaryTask = _group getVariable ['QS_primaryPressure_task',[]];
		if ((_primaryTask param [4,'']) isEqualTo 'FINAL_AIR' &&
			{(_primaryTask # 0) isEqualTo (missionNamespace getVariable ['QS_primaryPressure_epoch',-1])} &&
			{missionNamespace getVariable ['QS_primaryPressure_running',FALSE]} &&
			{!(missionNamespace getVariable ['QS_defendActive',FALSE])}) exitWith {FALSE};
		if (_role isNotEqualTo 'CAS' || {!canMove _aircraft} || {isTouchingGround _aircraft}) exitWith {FALSE};
		private _plane = _aircraft isKindOf 'Plane';
		private _bearing = _point getDir _aircraft;
		if ((_point distance2D _aircraft) < 50) then {_bearing = getDir _aircraft;};
		private _goal = _point getPos [([650,1600] select _plane) + random ([350,800] select _plane),_bearing];
		_goal set [0,100 max ((_goal # 0) min (worldSize - 100))];
		_goal set [1,100 max ((_goal # 1) min (worldSize - 100))];
		_goal set [2,[130,350] select _plane];
		private _until = _now + ([15,60] select _plane) + random ([10,40] select _plane);
		{_x doTarget objNull; _x doWatch objNull;} forEach (units _group);
		_group move _goal;
		_group setVariable ['QS_combatAir_egressUntil',_until,FALSE];
		_group setVariable ['QS_combatAir_groundCooldownUntil',_until,TRUE];
		_group setVariable ['QS_combatAir_busyUntil',_until,TRUE];
		_group setVariable ['QS_combatAir_target',objNull,FALSE];
		_group setVariable ['QS_combatAir_airUntil',0,FALSE];
		_aircraft flyInHeight ([130,350] select _plane);
		TRUE
	};
	if (_mode isNotEqualTo 'GROUP') exitWith {FALSE};
	if (_aircraft getVariable ['QS_AI_PLANE_fireMission',FALSE]) exitWith {TRUE};
	// The request/plane flags can clear before the scheduled finalizer reaches
	// EGRESS. Keep its movement lease until that helper cleanup releases it.
	if (_now < (_group getVariable ['QS_combatAir_groundUntil',0])) exitWith {TRUE};
	private _egressing = _now < (_group getVariable ['QS_combatAir_egressUntil',0]);
	if (diag_fps < 18 || {!canMove _aircraft} || {isTouchingGround _aircraft}) exitWith {FALSE};
	if (_now < (_group getVariable ['QS_combatAir_nextCheck',0])) exitWith {
		_egressing || {_role isEqualTo 'CAP'} || {!isNull (_group getVariable ['QS_combatAir_target',objNull])}
	};
	_group setVariable ['QS_combatAir_nextCheck',_now + 3,FALSE];
	private _targets = (_pilot targets [TRUE,5000,[WEST],45]) select {
		alive _x && {_x isKindOf 'Air'} && {canMove _x} && {!isTouchingGround _x} &&
		{!captive (effectiveCommander _x)} && {(side (group (effectiveCommander _x))) isEqualTo WEST} &&
		{(_x distance2D _center) < 6000} && {(_x distance2D (markerPos 'QS_marker_base_marker')) > 1000}
	};
	private _current = _group getVariable ['QS_combatAir_target',objNull];
	private _combat = []; private _transport = [];
	{
		private _info = [_x] call _fn_weaponInfo;
		private _type = toLowerANSI (typeOf _x);
		private _combatClass = _type in (missionNamespace getVariable ['QS_core_classNames_planeTypesCAS_lower',[]]) ||
			{_type in (missionNamespace getVariable ['QS_core_classNames_heliTypesCAS_lower',[]])};
		if (_x isKindOf 'Plane_Civil_01_base_F') then {_combatClass = FALSE;};
		private _class = [_combatClass,_info # 2,_info # 3] call _fn_classify;
		if (_class isEqualTo 'COMBAT') then {_combat pushBack [_aircraft distance2D _x,_x];} else {_transport pushBack [_aircraft distance2D _x,_x];};
	} forEach _targets;
	private _currentCombat = (_combat findIf {(_x # 1) isEqualTo _current}) >= 0;
	// Ground-request pacing never creates an air-defense blackout. A known
	// combat aircraft can interrupt departure or the ground-priority interval.
	// Actual accepted ground passes still finish through their native cleanup.
	private _airUntil = _group getVariable ['QS_combatAir_airUntil',0];
	if (_role isEqualTo 'CAS' && {_airUntil > 0} && {_now >= _airUntil}) then {
		_group setVariable ['QS_combatAir_airUntil',0,FALSE];
		if (!_currentCombat) then {
			_group setVariable ['QS_combatAir_groundPriorityUntil',_now + 90,FALSE];
			{_x doTarget objNull;} forEach (units _group);
			_group setVariable ['QS_combatAir_target',objNull,FALSE];
			_group setVariable ['QS_combatAir_busyUntil',0,TRUE];
			_current = objNull;
	...
	private _threat = _aircraft getVariable ['QS_combatAir_threat',[objNull,0]];
	private _selected = objNull;
	if (_now < (_threat # 1) && {(_threat # 0) in _targets}) then {_selected = _threat # 0;};
	// Keep a valid combat engagement stable. Reconsider transport pursuit
	// every 12 seconds so a newly detected fighter gets a prompt selection.
	private _until = _group getVariable ['QS_combatAir_commitUntil',0];
	if (!_currentCombat) then {_until = _until min (_group getVariable ['QS_combatAir_transportUntil',0]);};
	if (isNull _selected && {[_now,_until,_current in _targets,_sameActivity] call _fn_lease}) exitWith {TRUE};
	if (isNull _selected && {[_role,_now,_group getVariable ['QS_combatAir_groundPriorityUntil',0],count _combat,FALSE] call _fn_airAvailable}) then {
		// Departure is a ground-pass cooldown. Do not restart a routine
		// transport chase during it, but keep combat threats eligible.
		private _class = [count _combat,[count _transport,0] select _egressing,random 1] call _fn_pickClass;
		private _pool = [_transport,_combat] select (_class isEqualTo 'COMBAT');
		if (_pool isNotEqualTo []) then {
			if ((_pool findIf {(_x # 1) isEqualTo _current}) >= 0) then {_selected = _current;} else {_pool sort TRUE; _selected = (_pool # 0) # 1;};
		};
	};
	_group setVariable ['QS_combatAir_target',_selected,FALSE];
	if (!isNull _selected) exitWith {
		// One target commitment per pass. Engine flight, aiming, ammunition and
		// countermeasures remain in charge; no forced hits or velocity steering.
		if (_selected isNotEqualTo _current || {_now >= (_group getVariable ['QS_combatAir_commitUntil',0])}) then {
			private _deadline = [_now,_role,_group getVariable ['QS_combatAir_airUntil',0]] call _fn_airDeadline;
			_group setVariable ['QS_combatAir_commitUntil',_deadline,FALSE];
			if (_role isEqualTo 'CAS') then {_group setVariable ['QS_combatAir_airUntil',_deadline,FALSE];};
		};
		_group setVariable ['QS_combatAir_busyUntil',_group getVariable ['QS_combatAir_commitUntil',_now + 45],TRUE];
		_group setVariable ['QS_combatAir_egressUntil',0,FALSE];
		if (_selected isNotEqualTo _current || {_now >= (_group getVariable ['QS_combatAir_transportUntil',0])}) then {
			_group setVariable ['QS_combatAir_transportUntil',_now + 12,FALSE];
		};
		// Issue combat orders on acquisition or a bounded refresh, not every
		// decision tick. Let the native pilot finish turns and firing solutions.
		if (_selected isNotEqualTo _current || {_now >= (_group getVariable ['QS_combatAir_fireRefresh',0])}) then {
			_group enableAttack TRUE; _group setSpeedMode 'FULL'; _group setCombatMode 'RED';
			{_x enableAIFeature ['AUTOCOMBAT',TRUE]; _x doTarget _selected; _x doFire _selected;} forEach (units _group);
			_group setVariable ['QS_combatAir_fireRefresh',_now + 12,FALSE];
		};
		if (_selected isNotEqualTo _current) then {
			diag_log format ['[CombatAir] TARGET role=%1 type=%2',_role,typeOf _selected];
		};
		TRUE
	};
	if (!isNull _current) then {{_x doTarget objNull;} forEach (units _group);};
	if (_egressing) exitWith {TRUE};
	_group setVariable ['QS_combatAir_busyUntil',0,TRUE];
	if (_role isEqualTo 'CAP') exitWith {
		if (_now >= (_group getVariable ['QS_combatAir_nextPatrol',0])) then {
			private _goal = _center getPos [1200 + random 1000,random 360]; _goal set [2,700];
			_group move _goal;
			_aircraft flyInHeight 700;
			_group setVariable ['QS_combatAir_nextPatrol',_now + 60,FALSE];
		};
		TRUE
	};
	FALSE
},FALSE];
// COMBAT_AIR_CONTROLLER_END
...
// Added Code
	...
	// ROLE_SUPPORT_WATCHDOG: reuse this server loop; no per-AO observer scripts.
	...
	['ARTY_TICK'] call QS_fnc_aoPressure;
	// Only registered combat flights get the existing ~3-second AI pass.
	// Infantry retains its normal scheduler. Dead flights are pruned here;
	// no per-aircraft scheduled loop or independent lifetime is introduced.
	private _combatFlights = (missionNamespace getVariable ['QS_combatAir_groups',[]]) select {
		!isNull _x && {local _x} && {alive (leader _x)} && {(vehicle (leader _x)) isKindOf 'Air'}
	};
	missionNamespace setVariable ['QS_combatAir_groups',_combatFlights,FALSE];
	{['GROUP',_x] call QS_fnc_combatAir;} forEach _combatFlights;
// End Updated Code
	/*/Get general data/*/
...
// Added Code
				...
				// ROLE_SUPPORT_START: one allowance per prepared Primary AO.
...
// Added Code
					// ROLE_SUPPORT_END: cancel pending volleys before this AO's cleanup.
...
// Added Code
				private _primaryFinal = _QS_module_classic_pressure && {missionNamespace getVariable ['QS_classic_AI_active',FALSE]} && {!(missionNamespace getVariable ['QS_defendActive',FALSE])} && {((missionNamespace getVariable ['QS_primaryPressure_state',createHashMap]) getOrDefault ['factor',1]) <= 0};
// End Updated Code
...
//|							call _fn_enemyCAS;
// Updated Code
							if (!_QS_module_classic_pressure || {missionNamespace getVariable ['QS_defendActive',FALSE]} || {['NATIVE','CAS',_QS_module_enemyCAS_spawnDelay] call QS_fnc_aoPressure}) then {
								call _fn_enemyCAS;
								if (_QS_module_classic_pressure && {!(missionNamespace getVariable ['QS_defendActive',FALSE])}) then {['NATIVE_DONE',missionNamespace getVariable ['QS_enemyCasArray2',[]]] call QS_fnc_aoPressure;};
							};
// End Updated Code
						};
...
// Updated Code
							// Managed interception/egress altitude belongs to the flight controller.
							if (!(_QS_module_enemyCas_plane getVariable ['QS_AI_PLANE_fireMission',_false]) &&
								{_QS_serverTime >= ((group (currentPilot _QS_module_enemyCas_plane)) getVariable ['QS_combatAir_busyUntil',0])} &&
								{((group (currentPilot _QS_module_enemyCas_plane)) getVariable ['QS_combatAir_role','CAS']) isNotEqualTo 'CAP'}) then {
// End Updated Code
...
// Updated Code
								// Managed flights select known aircraft through their own role/priority
								// pass. The legacy all-player reveal/target loop is the fallback only.
								if (alive (currentPilot _QS_module_enemyCas_plane) &&
									{!((group (currentPilot _QS_module_enemyCas_plane)) getVariable ['QS_combatAir_managed',FALSE])}) then {
// End Updated Code
...
// Updated Code
											if ((vehicle _x) isKindOf 'Plane' && {!_primaryFinal || {(_x distance2D _QS_module_classic_hqPos) < 1200}}) then {
// End Updated Code
...
//|											if (!isNull _x) then {
// Updated Code
											if (!isNull _x && {!_primaryFinal || {(_x distance2D _QS_module_classic_hqPos) < 1200}}) then {
// End Updated Code
