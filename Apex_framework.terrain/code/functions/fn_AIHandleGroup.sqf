...
if (
	...
	// Final HQ orders are the fallback destination; a live interception or
	// accepted ground pass keeps ownership of the aircraft until it releases.
	private _airOwns = !isNil 'QS_fnc_combatAir' && {_grp getVariable ['QS_combatAir_managed',FALSE]} && {['GROUP',_grp] call QS_fnc_combatAir};
	if (!_airOwns) then {['GROUP',_grp,_uiTime,_fps] call QS_fnc_aoPressure;};
...
// Updated Code
			_x enableAIFeature ['AUTOCOMBAT',_grp getVariable ['QS_combatAir_managed',FALSE]];
// End Updated Code
		} forEach (units _grp);
...
// Added Code
// Reuse the local group pass for combat aircraft. Primary rotary CAS is
// identified by its existing provider registration; sector patrols are separate.
if (!isNil 'QS_fnc_combatAir') then {
	if (!(_grp getVariable ['QS_combatAir_managed',FALSE]) &&
		{_currentConfig_major isEqualTo 'AO'} && {_currentConfig_minor isEqualTo 'AIR_PATROL_HELI'} &&
		{((units _grp) findIf {_x in (missionNamespace getVariable ['QS_AI_supportProviders_CASHELI',[]])}) >= 0}) then {
		['REGISTER',_grp,'AO'] call QS_fnc_combatAir;
	};
};
if (_grp getVariable ['QS_combatAir_managed',FALSE] && {!isNil 'QS_fnc_combatAir'} && {['GROUP',_grp] call QS_fnc_combatAir}) exitWith {};
// End Updated Code
