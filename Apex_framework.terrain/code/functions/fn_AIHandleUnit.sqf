...
// Added Code
// Primary-only exceptions include tropical/ghillie sniper classes. Keep the
// original stance rules in other activities, including Defense.
...
// Added Code
// PRIMARY SIGHTINGS: initial patrols/guards and assigned defenders report real
// observations through the existing server/HC intel channel. At most three
// targets per leader per ten seconds; neither controller reveals hidden players.
...
// Added Code
// OUTGOING_SUPPRESSION_POLICIES_BEGIN
private _fn_coverReady = {
	params ['_fps','_now','_after','_active','_limit','_range','_age','_error','_ammo'];
	// FPS never disables covering fire. Keep range, age, ammo and slot limits.
...
private _fn_coverProfile = {
	...
	// Shooter limit, burst min/max, cooldown min/max. Keep the visual feature
	// at low turnout without sustained full-squad suppression of a few players.
	if (_players <= 8) exitWith {[2,4,6,60,90]};
	[[2,4] select (_members >= 8),8,12,24,36]
};
// MGs take the first covering slots; riflemen fill the remaining places.
// The caller excludes the leader, so the group retains its movement leader.
...
// OUTGOING_SUPPRESSION_POLICIES_END
// Covering fire uses the existing owner-local infantry pass. Two or four
// shooters fire 8-12 seconds at a recent observed position while others move.
// No projectile steering, extra ammunition, screen effects or new worker.
...
// Added Code
	if (_coverManaged && {!_isSuppressing} && {!captive _unit} &&
		...
		// Reject cooldown/ammo/slot failures before target and visibility queries.
		...
		if (_unit in _shooters && {_active < _limit}) then {
			...
			private _ordered = FALSE;
			{
				...
				if (_knowledge # 0 && {(_unit knowsAbout _x) >= 1.5} &&
					...
					private _ray = _point vectorDiff _start; private _length2 = _ray vectorDotProduct _ray;
					...
					if (_blocked < 0 && {!terrainIntersectASL [_start,_point]}) then {
						...
						_ordered = TRUE; _isSuppressing = TRUE;
					};
				};
...
//|					};
// Updated Code
				// Rank fresh observed assets for every support request, including an existing infantry engagement.
...
// Added Code
				...
				private _target = [_unit,_unit targets [TRUE,600,[WEST],30],_unit,600,time] call _fn_groundObservedPick;
// End Updated Code
				if (
					(alive _target) && 
...
// Updated Code
															if (_primaryAO) then {
																// targetKnowledge positions are ASL; carry the reported position,
																// not the target's live coordinates, with a server-time timestamp.
																private _knowledge = _unit targetKnowledge _target;
																_targetPos = ASLToAGL (_knowledge # 6); _targetPos set [2,0];
...
// Added Code
							// CAP, interceptions and egress are not available for a new ground pass.
							_supportProviders = _supportProviders select {isNil 'QS_fnc_combatAir' || {['AVAILABLE',_x] call QS_fnc_combatAir}};
// End Updated Code
							{
								_supportProvider = _x;
								if (!isNull _supportProvider) then {
