/*/
File: fn_SMpriorityAALegacy.sqf
Author:
...
} forEach _composition;
{
	if (alive _x) then {
		...
		_aaGroup setBehaviourStrong 'AWARE';
		...
		[_aaHull] call QS_fnc_airDefenseRegister;
...
[
	...
	TRUE,
	[
		localize 'STR_QS_Task_094_Legacy',
...
private _cleanupObjects = _enemyAssets + _composition + _aaHulls;
{
	(missionNamespace getVariable 'QS_garbageCollector') pushBack [_x,'NOW_DISCREET',0];
} forEach (_cleanupObjects arrayIntersect _cleanupObjects);
