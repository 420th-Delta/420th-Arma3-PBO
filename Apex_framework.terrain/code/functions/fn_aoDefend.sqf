...
// Added Code
// ROLE_SUPPORT_START: only a Defense that passed the native start gates rearms.
...
// Added Code
				if (!isNil 'QS_fnc_combatAir') then {['REGISTER',_grp,'DEFENSE'] call QS_fnc_combatAir;};
// End Updated Code
				_jetSpawnDelay = time + 60 + (random 60);
			};
		};
...
// ROLE_SUPPORT_END: cancellation, success and failure all close the same allowance.
