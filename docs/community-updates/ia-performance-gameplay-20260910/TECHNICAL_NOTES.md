...
The sections are grouped around behaviors that should be tested together. File names below are relative to the mission root; unqualified function names are in `code/functions/`. The full file-by-file mapping and test scenarios are in the Files and testing section of `RELEASE_NOTES.md`. Several features share complete runtime files: the 46-file Gameplay bundle is the common dependency for sections 1–8 and the background cleanup in section 10. The seven Radio files, two BLUFOR speech files, one vehicle-handler fix and three shared performance helpers have independent baseline test boundaries, followed by combined acceptance with all 59 files.
...
### Four-minute incapacitated window

`description.ext` sets `ReviveBleedOutDelay = 240`. The supplied original was 600. `fn_incapacitated.sqf` reads this mission setting, derives its local deadline and published bleed-out time, displays the remaining time, and forces respawn on expiry. The existing timeout loop remains.

The existing code grants 60-second extensions near expiry while a casualty is aboard/attached to a living crewed vehicle. Requested AI medevac can replace the deadline with its extraction timeout. These paths remain; four minutes is the normal unattended window, not an absolute maximum during an extraction. The mission source contains no second assignment overriding `ReviveBleedOutDelay`. Verify with the deployed revive/mod setup.
...
**Independent radio bundle:** `fn_config.sqf`, `fn_initPlayerLocal.sqf`, `fn_clientEventRespawn.sqf`, `fn_clientRadio.sqf`, `fn_clientMenuRadio.sqf`, `fn_highCommand.sqf` and `TGC/Functions/Channels/fn_refreshStaffChannelAccess.sqf`.
...
The original live mission used native faction Side transmission and an optional General subscription. The update creates an optional cross-team Side channel, makes General membership mandatory and keeps the private Staff channel separate.
...
All ten existing custom channel definitions are preserved. The optional Side colour is read from `CfgInGameUI >> Chat >> colorSideChannel`, matching the game's native Side colour. A malformed/missing colour array falls back to the existing blue so channel allocation remains valid. A separate custom channel is created once in `fn_config.sqf` and published as `QS_radioChannel_side`. The Radio Management first row uses that ID instead of the private Staff channel. The engine needs capacity beyond the original ten slots (Arma 3 2.22+). Custom IDs 1–10 map to UI IDs 6–15; IDs 11–50 map to 26–65. The allocated ID is used rather than assuming it is 11. If allocation fails, the Side checkbox is unavailable; native Side sending remains disabled, and General/other channels remain available. Check allocation during acceptance.

`QS_client_radioChannel_side` is a new boolean in the client's `missionProfileNamespace`, default true. Only changing the Side checkbox saves it. The existing `missionGroup = "ApexFramework"` keeps the profile namespace compatible across mission updates. Normal server resets do not erase a client's saved preference; changing/deleting the local profile can. The obsolete optional General preference no longer controls membership.

`fn_refreshStaffChannelAccess.sqf` retains the original private Staff rule separately from General voice eligibility. The existing one-second radio loop, initialization, respawn, High Command entry/exit and late whitelist response call the same reconciler. It caches player identity, actual group/team, authorization, allocated Side ID and Side preference; unchanged checks do not rewrite channel membership or permissions. No extra scheduler, remote-execution endpoint or whitelist privilege is introduced. Existing TGC channel masks continue to combine with desired text/voice permissions and cannot grant non-admin/non-Zeus General voice.

General cannot be removed through the subscription helper or a stale checkbox event. Death retains its subscription; respawn explicitly removes memberships from the old body, restores the new body and reapplies authorization. Side opt-out does not remove Staff, General or other custom channels.

The native `enableChannel` control governs player sending. Genuine per-player receive opt-out therefore uses custom-channel membership, leaving scripted Crossroads radio messages on their existing path. The expected player selector contains one usable Side channel; actual selector, voice delivery, death-screen reception and profile disk persistence require the two-client acceptance cases.

The custom Side channel shares player text and voice across BLUFOR, OPFOR, Independent and Civilian. Membership follows the saved subscription, not faction. The existing one-second access check includes actual group identity and group side; a switch reapplies receive membership even if the local subscription roster still lists Side. Stable checks perform no membership or permission writes. Native Side remains closed to player sending; scripted Crossroads traffic retains its existing mission routing. General permissions and private Staff eligibility are unchanged.
