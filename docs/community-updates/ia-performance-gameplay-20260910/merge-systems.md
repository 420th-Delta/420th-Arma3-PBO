...
Contributor feedback later placed the shared Side/staff General policy behind `QS_missionConfig_sharedRadioChannels`, a startup-only gate that defaults off. The merge resolutions above remain the provenance of the opt-in implementation. Current default behavior skips the extra allocation, keeps native Side and optional General semantics, and uses the original High Command transitions; the corrective staff GUI also omits the opt-in Side mask. The exact-target body handling and donor entitlement correction remain common to both modes.
