...
The radio-cleanup fixture first runs with `QS_radio_sharedBroadcastsEnabled` absent or false. It verifies that slot 11 is initially free, then checks native Side permissions, private Staff, optional General, legacy staff masks and exact-body cleanup. The fixture allocates its own channels and does not execute production `fn_config.sqf`; source inspection establishes that the production false branch skips `radioChannelCreate`. It next explicitly enables the retained shared-channel design in isolation and reruns its custom Side, mandatory General, authorization and delayed replacement cases. This does not toggle a live mission. Player menu/selector behavior, production startup/publication, real respawn, High Command, JIP/cross-team reception, profile reload, voice audibility and the eventual redesigned layout remain integration or human acceptance work.

The support fixture includes a three-mortar incapacitation cycle. It verifies that menus disappear, both the ordinary reset callback and a crafted direct server RESET are denied, nearby equipment remains, menus return after recovery and an authorized reset then succeeds. The integration suite reads `ReviveBleedOutDelay` through the engine's mission configuration and requires the final value of 300 seconds.
